...
If a `CONTEXT.md` is being created for the first time, also check if `agents/guidelines/` directory exists at project root.
- If no guideline file relevant to the project's primary language or stack exists, propose creating one.
- Guideline files live at `agents/guidelines/<topic>.md`.
- A guideline file documents language/framework conventions specific to this project (e.g. naming, patterns, anti-patterns).
- Do not create a guideline file automatically — propose it to the user and wait for confirmation.
